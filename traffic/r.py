...
def create_detections(results, labels, model_type="yolov8", yolo_v8_thresh=0.75):
        ...
        if hasattr(results, 'boxes'):
            for box in results.boxes:
                ...
                if label in emergency_labels + non_emergency_labels:
                    print(f"[YOLOv8] Label: {label}, Confidence: {conf:.2f}")
                    detections.append(
                        Detection(
                            centroid, 
...
if uploaded_files:
    for idx, uploaded_file in enumerate(uploaded_files[:4]):  # Process up to 4 videos
        ...
        st.write(f"Final Non-Emergency Vehicles: {len(unique_non_emergency_ids)}")
